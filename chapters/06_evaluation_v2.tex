...
\todo{ ``In the end, we would like to answer, if it make sense to evaluate this method using degenerated type usages as proposed by Monperrus et al.'' }
...
\todo{a bit more than just ``small partition?''}
...
\todo{ rephrase end of this paragraph - the second part does not actually follow!  }
...
\begin{itemize}
    \item suggesting a missing call to \code{<init>} (basically never valid, usually the object is a parameter of the method or created by a static function)
...
\end{itemize}
Some of these could be fixed rather easily, e.g., we could filter all type usages related to \code{StringBuilder} or anomalies that suggest a call to \code{<init>} is missing.
...
\todo{somewhere give a GENERAL aussage like: its good or not so much!}
...
\todo{ not super happy with the end\ldots}
...
We think that the Android framework should be a favorable for this method because of its focus on the user interface, but it is feasible that this method works much better on another OOP language or simply another Java framework.
Finally, it is not clear if open source applications are comparable to commercial software in terms of size or quality.

\todo[inline]{not yet perfect but not sure how to change it\ldots (letzter absatz)}
