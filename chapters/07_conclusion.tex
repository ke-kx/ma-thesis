...
% MMCS
As for detecting missing method calls
    results of benchmark and manual review agree that the original version proposed by mp is the best
    other methods turned out to be inferior!
take answers of all the questions into account 
 oh its a bit sensible to wrong inputs and it is difficult to gather enough data - result quality is also not amazing - 
    results quality? -> ``okay'' but not amazing (detected one bug, that's it)
    input size: it is difficult to gather enough data even when using lots of input
    additionally method is sensible to erroneous / perturbed input data
lots of fp, especially among top50
all in all it seems that at least in the context of android this is A method for detecting mmcs, but probably not the best (cant say that without example i gues..)
tentively that it's inferior because of bad detection rate oä
could be a solution with the filters, but then you get the same problmes as before
(i mention this below, is that fine?)

% as for other anomalies
as for detecting superfluous or wrong method calls
our simple ideas do not seem to work well
manual review of results found all of the anomalies to be Fps!
