...
\todo[inline]{
    ...
    do i wann use the benchmark for anything else?
        like smth just about benchmark performance relative to no tus in partition?
}

\subsection{RQ3: How robust is the Majority Rule in the Face of erroneous Input Data?}

\todo[inline]{
worauf is robustness bezogen? -> results quality or smth else? (Robustness explanation)

not super happy with the questions in description, is it fine?
}
...
%--- in detail smth about each question? (aka using the 10 rnd apps, or not or \ldots) or this only necessary later?
...
\todo[inline]{per RQ one part explaining the GENERAL idea}
...
\todo[inline]{this below here or in procedure?}
