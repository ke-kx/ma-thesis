...
Unfortunately, we do not have the resources to let multiple review the anomalies.
...
On the other hand, we can also exclude hints and obtain a more strict assessment.

The only parameter that our implementation uses is the strangeness value that indicates that a type usages is an anomaly.
During the manual review, we often observed that even type usages with a strangeness value much higher than $0.9$ were not really true positives.
It even seems like anomalies with a strangeness value smaller than $0.95$ are less and less likely to hint at actual problems.
\todo{smth like ``thisis just an observtion, no actual data''?}
That is, we deem it unlikely that reducing the cutoff to a lower value would improve the results.

-> other direction set cutoff higher, less findings
In the other direction, we think that the average number of findings is fine 
